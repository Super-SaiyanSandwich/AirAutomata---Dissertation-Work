...
for j in range(locs):
	d[j] = list(scaler.transform(array(sec[j]).reshape(-1,1)))

d = array(d)
e = d.reshape(locs,data.shape[0]).T
...
bigY = array([array([ [max(a[:,(i*locs) + j]) for j in range(locs)] for a in data]) for i in range(1)])
...
model = Model(inputs=[i], outputs=[outputLayer1])
